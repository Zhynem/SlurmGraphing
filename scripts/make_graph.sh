...
"jobs")rrdtool graph /rc/hyperion/SlurmGraphing/server/html/graphs/slurm-$NAME-$TYPE-$PERI.png -z --logarithmic --units=si --lower-limit 1 -r --full-size-mode --width 500 --height 200 --start $BEG --end $END --step 60 --title "$NAME:partition:$PERI" --color CANVAS#A0A0A0 DEF:queued=$PRRD:queued:LAST DEF:running=$PRRD:running:LAST AREA:queued#B30000:"Queued" STACK:running#0200B3:"Running" >> $log_loc/graph.log >&2;; 
